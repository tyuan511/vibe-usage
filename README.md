...
- **随时查看今日用量**：菜单栏图标旁以两行显示今日费用和 Token，也可以在设置中只保留图标。
...
|---|---|---|
| Claude Code | `~/.config/claude`、`~/.claude` | `CLAUDE_CONFIG_DIR`、`XDG_CONFIG_HOME` |
...
VibeUsage 在相应目录中找到可识别的会话文件后，会自动发现该 Agent。除 `XDG_CONFIG_HOME` 外，表中的位置环境变量均支持用逗号分隔多个路径；Claude Code 未设置 `CLAUDE_CONFIG_DIR` 时，会同时尝试 `$XDG_CONFIG_HOME/claude`。修改环境变量后请退出并重新打开 VibeUsage。
...
费用统一以美元显示。日志中已有费用时优先使用原值，否则根据当前模型价格估算。VibeUsage 不是官方账单，结果仅供日常观察使用。
...
模型价格更新成功后，VibeUsage 会重新计算已标记为估算的历史费用。应用也会自动检查新版本，并在发现更新时在面板底部显示提示；安装更新时可能需要 macOS 授权写入“应用程序”目录。
...
特别感谢 [ccusage/ccusage](https://github.com/ccusage/ccusage)。VibeUsage 对各类 Agent 的 Token 用量统计与日志解析逻辑参考了该项目的成熟实践，并在此基础上针对 macOS 菜单栏、后台增量扫描、额度监控和多设备使用场景进行了设计。
