...
菜单栏下拉顶部可以把当前筛选范围一键导出成分享海报（保存图片 / 拷贝图片 / 系统分享）：
...
- 订阅额度监控（可选）：在菜单栏点击"连接 Claude / Codex 账号"，走各自官方 OAuth 授权流程后，显示 Claude Code（5 小时会话 / 7 天总量 / 7 天 Opus / 7 天 Sonnet）和 Codex/ChatGPT（当前会话 / 每周额度）的剩余额度和重置倒计时；可随时断开连接，或在设置中整体关闭（默认开启）。
- 支持按今天、昨天、本周、本月筛选。
- 支持隐藏/显示已发现的 agent 来源。
- 菜单栏下拉顶部可导出/分享当前时间、模型与 Agent 筛选范围的统计图片。
- 菜单栏图标旁可选择显示今日花费、今日 Token 或只显示图标，并与下拉共用 Agent 统计范围。
- 原生设置窗口集中管理菜单栏指标、Agent 统计范围、订阅额度显示与联网监控。
...
- `VibeUsageAggregation`：把存储层数据聚合成 UI 和分享海报使用的 snapshot。
...
- `VibeUsageUI`：SwiftUI 菜单栏、设置窗口与分享海报视图。
- `VibeUsageApp`：应用入口、依赖装配和 Sparkle 应用内更新。
...
- `CI`：在 push / pull request 时运行 `swift test`；`main` 分支 push 后会尝试自动更新 `docs/usage-share-preview.png`。
...
应用会自动检查新版本，也可以在菜单栏下拉窗口底部点击“检查更新…”。发现更新后，Sparkle 会在应用内下载、验证、替换并重启应用；需要写入 `/Applications` 时 macOS 可能要求授权。
