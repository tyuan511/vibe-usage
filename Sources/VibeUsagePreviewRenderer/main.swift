...
struct VibeUsagePreviewRenderer {
    static func main() throws {
        ...
        let outputPath = arguments.first(where: { !$0.hasPrefix("--") })
            ?? ((isShareCard || isShareCardSparse) ? "share-card-preview.png" : (isDashboard ? "dashboard-preview.png" : "docs/usage-preview.png"))
        ...
        if isShareCardSparse {
            ...
            try renderMenuPreview(outputPath: outputPath)
        }
    }

    // MARK: - Menu bar preview (default; used by Scripts/regenerate-preview.sh)
    ...
    private static func renderMenuPreview(outputPath: String) throws {
        ...
        try renderPNG(view: view, size: size, scale: 2, outputPath: outputPath)
    }
...
private func makeMenuSnapshot() -> UsageDashboardSnapshot {
